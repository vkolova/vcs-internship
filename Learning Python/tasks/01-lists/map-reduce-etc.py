...
def task3(a, b):
    """
    От зададен интервал да се изведе речник,
    съдържащ двойки ключ:стойност,
    където ключът е пореден елемент от интервала,
    а стойността е остатъкът при деление на 5 на
    същия този елемент:
    function(3,5) --> {3:2, 4:1, 5:0}
    """
    print {a: a % 5 for a in range(b + 1)}
